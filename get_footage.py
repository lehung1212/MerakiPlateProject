...
from meraki_helpers import get_network_id, req, encode_image
import base64


os.environ["MERAKI_API_KEY"] = '595080bbcdf274431859183b10b74721d7207c64'
os.environ["MERAKI_ORG_NAME"] = 'Alvin\'s Dashboard'
os.environ["MERAKI_NET_NAME"] = 'OJE'
...
def main(org_name, net_name, timestamp):
    ...
    for camera in cameras:
        ...
        #encode base 64 image and write to snapshot folder:
        load_image = open(f"{snapshot_dir}/{sn}.jpg", 'rb')
        en64_image = base64.b64encode(load_image.read())
        en64_image = en64_image.decode('utf-8')
        with open(f"{snapshot_dir}/{sn}.txt", "w") as handle:
            handle.write(en64_image)
        print(f"encoded snapshot for camera {sn} saved")
